  ...
  return (
    <div className="space-y-6">
       <div className="flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold text-slate-100">Fila de Aprovação (KYC)</h2>
            <p className="text-slate-400 text-sm">Analise e aprove a documentação de novos clientes.</p>
          </div>
          <button onClick={fetchPending} className="p-2 text-slate-300 hover:bg-emerald-950/40 rounded-lg border border-emerald-500/20 transition-colors">
      ...
      <div className="grid grid-cols-1 gap-4">
        {isLoading ? (
          <div className="flex justify-center items-center py-20">
           ...
           <div className="text-center py-20 bg-[#0f1713] rounded-2xl border border-emerald-500/20 border-dashed">
             <div className="w-16 h-16 bg-[#090d0a] rounded-full flex items-center justify-center mx-auto mb-4">
               <Check className="w-8 h-8 text-green-500" />
             </div>
             <h3 className="text-lg font-medium text-slate-100">Tudo limpo!</h3>
             <p className="text-slate-400">Nenhuma solicitação de KYC pendente no momento.</p>
           </div>
        ) : (
          safePendingUsers.map((user) => (
            <div key={user.id} className="bg-[#0f1713] rounded-xl border border-emerald-500/20 shadow-sm animate-in slide-in-from-bottom-2 duration-300">
              <div className="p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div className="flex items-start gap-4 flex-1">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-slate-100">{user.name || 'Sem Nome'}</h3>
                    ...
                    <div className="mt-2 flex gap-2">
                        <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-[#0f1713] text-slate-300 border border-emerald-500/10 border border-emerald-500/20 uppercase">
                ...
                <div className="flex items-center gap-2 w-full md:w-auto mt-4 md:mt-0">
                  <button 
                    ...
                    className="px-3 py-2 text-slate-300 border border-emerald-500/30 rounded-lg text-sm font-medium hover:bg-[#090d0a] flex items-center justify-center gap-2 transition-colors"
                  >
              ...
              {expandedUser === user.id && (
                <div className="border-t border-emerald-500/20 bg-[#090d0a] p-6">
                  {loadingDocs[user.id] ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {(userDocuments[user.id] || []).map((doc) => (
                        <div key={doc.id} className="bg-[#0f1713] rounded-lg border border-emerald-500/20 p-4 hover:shadow-md transition-shadow">
                          <div className="flex items-start justify-between mb-3">
                            <div className="flex-1">
                              <h4 className="font-semibold text-slate-100 text-sm">{doc.document_type.toUpperCase()}</h4>
