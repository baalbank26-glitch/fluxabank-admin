...
export const Settings: React.FC = () => {
  ...
  if (isLoading) {
    return <div className="flex justify-center p-20"><Loader2 className="animate-spin w-8 h-8 text-emerald-500"/></div>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`rounded-2xl border shadow-sm overflow-hidden transition-all duration-300 ${maintenance.isActive ? 'bg-gradient-to-br from-emerald-950/20 via-white to-orange-50 border-emerald-500/30 shadow-emerald-500/30/40' : 'bg-[#0f1713] border-emerald-500/20'}`}>
        ...
        <div className="p-6 space-y-6">
          <div className="flex items-center justify-between">
            <div className="pr-4">
              <p className={`font-medium text-lg ${maintenance.isActive ? 'text-fluxabank-900' : 'text-slate-100'}`}>
                Modo Manutenção
              </p>
              <p className={`text-sm mt-1 ${maintenance.isActive ? 'text-emerald-600' : 'text-slate-400'}`}>
                {maintenance.isActive 
            ...
            <button 
              ...
              className={`
                ...
                ${maintenance.isActive ? 'bg-emerald-500' : 'bg-slate-200'}
            ...
            >
              ...
              <span
                className={`
                  pointer-events-none inline-block h-7 w-7 transform rounded-full bg-[#0f1713] shadow ring-0 transition duration-200 ease-in-out
              ...
              <div className="rounded-3xl border border-emerald-500/30 bg-gradient-to-br from-emerald-950/20 via-white to-orange-50 shadow-[0_20px_60px_-30px_rgba(8,145,178,0.45)] overflow-hidden">
                <div className="border-b border-emerald-950/40 bg-gradient-to-r from-emerald-500 to-emerald-600 px-6 py-5 text-white">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.24em] text-emerald-950/40">Modo ativo</p>
                      <h4 className="mt-2 text-xl font-bold">Configuração do aviso e exceções</h4>
                      <p className="mt-1 max-w-2xl text-sm text-emerald-950/40/90">
                    ...
                    <div className="rounded-2xl border border-white/20 bg-white/10 px-3 py-2 text-right backdrop-blur-sm">
                      <p className="text-[11px] uppercase tracking-[0.24em] text-emerald-950/40">Status</p>
                ...
                <div className="grid gap-5 p-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
                  <div className="rounded-2xl border border-emerald-950/40 bg-white/90 p-5 shadow-sm">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <input
                        ...
                        className="flex-1 rounded-2xl border border-emerald-500/30 bg-emerald-950/20/40 px-4 py-3 text-slate-100 outline-none transition-all placeholder:text-slate-400 focus:border-emerald-500 focus:bg-[#0f1713] focus:ring-4 focus:ring-emerald-950/40"
                        placeholder="Ex: Sistema em manutenção até as 14h."
                      />
                      <button
                        onClick={handleUpdateMessage}
                        disabled={isSaving}
                        className="inline-flex items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-5 py-3 font-medium text-white shadow-lg shadow-emerald-600/20 transition-all hover:bg-emerald-600 disabled:cursor-not-allowed disabled:opacity-60 whitespace-nowrap"
                      >
                  ...
                  <div className="rounded-2xl border border-emerald-950/40 bg-slate-950 p-5 text-white shadow-sm">
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {parsedAllowedUserIds.length > 0 ? (
                          parsedAllowedUserIds.map((id) => (
                            <span
                              key={id}
                              className="rounded-full border border-emerald-400/30 bg-emerald-500/15 px-3 py-1 text-xs font-semibold text-emerald-950/40"
                            >
                              ID {id}
                            </span>
                          ))
                    ...
                    <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <button
                        ...
                        disabled={isSaving}
                        className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#0f1713] px-5 py-3 font-medium text-white transition-all hover:bg-emerald-950/40 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                        {isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                        Salvar IDs
                      </button>
      ...
      <div className="bg-[#0f1713] rounded-2xl border border-emerald-500/20 shadow-sm overflow-hidden opacity-60 pointer-events-none grayscale">
        <div className="p-6 border-b border-emerald-500/20">
          <h3 className="font-bold text-lg flex items-center gap-2 text-slate-100">
            <Shield className="w-5 h-5 text-slate-400" /> Segurança (Em breve)
          </h3>
        </div>
        <div className="p-6 space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-medium text-slate-100">Autenticação 2FA Global</p>
              <p className="text-sm text-slate-400">Forçar 2FA para todos os administradores.</p>
            </div>
